...
async def main():
    async with get_db_context() as session:
        ...
        print("Fetching 1m candles for BTCUSDT...")
        # Fetching 1m is too much for a single call if there are 1M+ rows
        # Let's fetch in batches or just use a sample month to verify signal frequency
        
        month_end = datetime(2024, 2, 1, tzinfo=timezone.utc)
        ...
        print(f"Fetched {len(candles)} 1m candles for January 2024.")
        if not candles:
            return
        ...
        print(f"\nPotential signals in January 2024: {len(signals)}")
        for s in signals:
